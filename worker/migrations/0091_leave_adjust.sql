...
-- a standing plus/minus on what a person is eligible for.
--
-- Eligible days are CALCULATED, never stored: entitlement times months
-- elapsed, minus what has been taken. That is the right default, and it is
-- what stops a new joiner taking a full year in February, but it left no way
-- to record the ordinary exceptions a company actually has:
--
--   * days carried forward from last year
--   * a one-off grant for covering a weekend
