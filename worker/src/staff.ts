...
export function paymentDateFor(payMonth: string): string {
  ...
  if (!Number.isInteger(py) || !Number.isInteger(pm) || py === undefined || pm === undefined || pm < 1 || pm > 12) throw new Error("Invalid payroll month");
...
export async function handleStaff(
  ...
  // Authenticated file export. The portal chooser also uses this shared builder.
  {
  ...
  if (path === "/payroll" && method === "GET") {
    ...
    return json({
      ...
      release: {
        ...
        employer_is_legacy: releasedRow !== null && !releasedRow.issuer_code,
      },
    });
  }
