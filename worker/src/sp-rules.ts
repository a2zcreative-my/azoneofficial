/**
 * SALES PERFORMANCE - THE RULES, as pure functions. v1.155.0.
 *
 * Everything on this file is decided the same way on the worker and in the
 * browser: lib/sales-performance.ts is the browser's copy, and
 * tests/sales-performance.mjs bundles both and runs them side by side over
 * the same inputs, so the two cannot drift. The worker's answer is the one
 * that counts - the browser's is a courtesy, so a typist hears about a
 * platform mismatch before pressing Save.
 *
 * WHAT LIVES HERE
 *   - the closed vocabularies (platforms, interaction types, statuses,
 *     activity kinds) with their labels in both languages;
 *   - the social URL rules: which platform a URL belongs to, the IDENTITY of
 *     a post once tracking parameters, case, www, trailing slashes and the
 *     mobile/short hosts are stripped (so the same post cannot be submitted
 *     twice in two spellings), and the account handle where the platform
 *     puts it in the URL;
 *   - the productivity score: seven weighted components, banded.
 *
