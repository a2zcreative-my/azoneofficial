...
/* ---- 5. the change is recorded, reported, and the column exists ---- */
{
  ...
  /* (which migration is LATEST is registry-parity's to check - pinning it
     here made this guard fail the day 0114 landed, for nothing) */
  ok("0113 has a health probe", /\["0113 \(who reports to whom\)"/.test(index));
...
else console.log(`PASS — nobody falls off the chart, a loop cannot hang it, the three the CEO named, and every change recorded (${passed} checks)`);
