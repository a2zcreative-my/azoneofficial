...
/* ---- 4. one tab, one page ---- */
{
  ...
  ok("work and daily closing precede reporting in the approved order", ["sp-feed", "sp-posts", "sp-engagements", "sp-promotions", "sp-orders", "sp-shipments", "sp-closing", "sp-funnel", "sp-trend"].every((id, i, a) => i === 0 || panel.indexOf(`id="${a[i - 1]}"`) < panel.indexOf(`id="${id}"`)));
  ok("the whole page is ONE read, remembered and live", /useCachedApi<Overview>\(`\/staff\/sales-performance\/overview\?v=\$\{SHAPE\}&\$\{qs\}`, true, TOPICS\)/.test(panel) && /const TOPICS = \["sales-performance", "docs", "postage"\]/.test(panel));
  /* v1.155.1 - the CEO's screenshot: "Cannot read properties of undefined
     (reading 'length')" - a remembered answer from an older cut drawn by a
     newer page. The read is keyed by SHAPE and every field is normalised. */
  ok("a remembered answer from an older cut cannot crash the page: keyed by shape, normalised before use", /const SHAPE = \d+;/.test(panel) && /const ov = useMemo\(\(\) => normalizeOverview\(view\.data\), \[view\.data\]\);/.test(panel) && /tiktok_orders: arr<TikTokOrder>\(raw\.tiktok_orders\)/.test(panel) && /feed: arr<Feed>\(raw\.feed\), posts: arr<Post>\(raw\.posts\)/.test(panel) && /team: \{ figures: figs\(raw\.team\?\.figures\)/.test(panel));
  ok("...and no other list on the page is read straight off view.data", !/view\.data\.\w+\.(length|map)/.test(panel));
  ...
  ok("TikTok Shop orders are shown, credited, and folded into the sales figures on the page", /TikTok Shop orders/.test(panel) && /o\.staff_names\.join\(" \+ "\)/.test(panel) && /t!\.tiktok_cents/.test(panel) && /f\.tiktok_cents > 0 &&/.test(panel));
  /* every row's actions are built once (an Act[]) and drawn on desk and phone;
     a Verify act is only ever pushed under `manager && !own` */
  ok("Verify is drawn only for management on someone else's record", (panel.match(/if \(manager && !own[^\n]*L\("Verify", "Sahkan"\)/g) ?? []).length === 3 && /if \(manager && !mine\(o\.user_id\)[^\n]*L\("Verify", "Sahkan"\)/.test(panel) && (panel.match(/L\("Verify", "Sahkan"\)/g) ?? []).length === 4);
  ok("every list is drawn for the phone AND the desk from the same rows", (panel.match(/<ul className=\{phoneList\}>/g) ?? []).length >= 8 && (panel.match(/<div className=\{deskTable\}>/g) ?? []).length >= 8 && /const phoneList = "divide-border divide-y md:hidden";/.test(panel) && /const deskTable = "hidden overflow-x-auto md:block";/.test(panel));
  ok("phone actions are thumb-sized, two to a row; desk actions are row buttons", /h-10 items-center justify-center rounded-xl/.test(panel) && /phone \? "mt-2 grid grid-cols-2 gap-1\.5" : rowActions/.test(panel));
  ok("the drawer is a bottom sheet on the phone, respecting the safe area", /items-end justify-center[^"]*sm:items-center/.test(panel) && /pb-\[calc\(1\.25rem\+env\(safe-area-inset-bottom\)\)\]/.test(panel));
  ok("the filters stack two to a row on the phone", /grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center/.test(panel));
   ...
   opens the register on that person and that day. */
{
  ...
  ok("the Dashboard quick action and the palette action into Sales follow the strip, not the role list alone", /\(canOpen \? canOpen\("Sales"\) : SALES_ROLES\.includes\(user\.role\)\) && \(/.test(dash) && /<Dashboard user=\{user\} go=\{setTab\} canOpen=\{canOpen\} lang=\{lang\}\s+onCreateQuotation=/.test(page) && /\.\.\.\(canOpen\("Sales"\)\n\s+\? \[/.test(page) && !/SALES_ROLES\.includes/.test(page));
}
