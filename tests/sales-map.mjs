...
/* ---- 5. the wiring ---- */
{
  ...
  /* Approved work-first layout: the map follows documents and customers. */
  const salesTab = page.slice(page.indexOf('{activeTab === "Sales" && ('), page.indexOf('{activeTab === "Content" &&'));
  ok("the panel stays lazy and follows the Sales workspace", /SalesMap = lazy\(/.test(lazy) && salesTab.indexOf("<Sales user={user}") > 0 && salesTab.indexOf("<SalesMap />") > salesTab.indexOf("<Sales user={user}"));
...
else console.log(`PASS — the map places every ringgit it can and says what it could not (${passed} checks)`);
