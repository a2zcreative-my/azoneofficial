...
ok("globals.css imports the V3 layer right after Tailwind", /@import "tailwindcss";[\s\S]{0,600}?@import "\.\/erp-v3\.css";/.test(globals));
const CLASSES = [
...
const MAP = {
  ...
  inputClass: "erp-input", inputClassSm: "erp-input erp-input-sm", selectClass: "erp-input erp-select sm:w-auto",
...
const rawVocab = [...styles.matchAll(/export const (\w+) =\s*(?:\/\*[\s\S]*?\*\/\s*)?"([^"]*)";/g)]
  .filter(([, name, val]) => !["PORTAL_WIDTH", "mobileAppBottomClearance", "mobileBottomNav", "inputClassLg", "btnHdrDesktop"].includes(name))
...
ok("the claim type switch is the V3 segmented control", /className="erp-segmented mt-3" role="group"/.test(claims));
...
ok("the bottom nav pads by the home-indicator inset", /export const mobileBottomNav\s*=\s*\n?\s*"[^"]*pb-\[max\(env\(safe-area-inset-bottom,0px\),6px\)\][^"]*"/.test(styles));
ok("the page clears the nav by the same formula", /export const mobileAppBottomClearance\s*=\s*\n?\s*"pb-\[calc\(5rem\+max\(env\(safe-area-inset-bottom,0px\),6px\)\)\]"/.test(styles));
...
for (const g of ["interface-system", "card-vocabulary", "status-tokens", "users-ui", "pwa-calendar", "shell-scroll", "registry-parity", "production-safety", "hankeis", "csv-export"]) {
...
ok("the registry grew to at least 92", names.length >= 92, String(names.length));
