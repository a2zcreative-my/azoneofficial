...
ok("a clock-in is refused only while one is OPEN",
   /if \(body\.type === "clock_in" && openNow\) \{/.test(staff),
   "refusing a second clock-in per day is the rule this version removes");
...
ok("Clock in is offered whenever nothing is open", /disabled=\{!!busy \|\| openNow\}/.test(dash) && /Clock in · next shift/.test(dash));
ok("the OT buttons are gone", !/punchOt\(/.test(dash) && !/"OT in"/.test(dash) && !/"OT out"/.test(dash) && !/ot_in/.test(dash));
ok("the card names today's shifts, every block, and says the rule",
   /Today's shifts: \$\{todayShift\.label\}/.test(dash) && /Syif hari ini/.test(dash) && /sent to the CEO as overtime automatically/.test(dash));
