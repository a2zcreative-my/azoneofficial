...
for (const tab of TABS) {
  ...
  if (/Something went wrong|Ada masalah pada skrin ini/.test(txt)) { const d=(txt.match(/(?:Detail for support|Butiran untuk sokongan): (.*)/)||[])[1]; findings[tab]=['<CRASHED> '+(d||'')]; await p.goto(`${process.env.PORTAL_BASE ?? 'http://localhost:8931'}/portal.html`,{waitUntil:'domcontentloaded'}); await p.waitForTimeout(2500); continue; }
