...
export interface CoverflowItem {
  /** Stable key. */
  id: string;
  /** Optional destination for the centre card. */
  href?: string;
  imageSrc?: string;
  imageAlt: string;
  /** Caption under the centre card. */
  label: string;
  /** Small chip on the card, e.g. a category. */
  badge?: string;
}
 ...
 */
/**
 * CoverflowGallery — centre card prominent, neighbours peeking behind.
 * Client-agnostic: pass any items with an image and a label.
 */
  ...
  return (
    ...
    >
      ...
      <p className="sr-only" aria-live={autoPlay && !paused ? "off" : "polite"}>
        Showing item {active + 1} of {count}: {products[active]?.label}
      </p>
    </div>
  );
}
