...
| Date | Version | Change |
| --- | --- | --- |
| 2026-08-24 | 1.44.0 | `web_orders.marketing_consent`. |
...
## v1.100.0 — migrations 0111_hotels.sql and 0112_hotels_seed.sql
The hotel directory the CEO supplied as 1. DATA HOTEL.xlsx. `hotels` (state,
hotel_name, company, address, rooms, stars, mof_validity, halal_validity,
notes, is_active, created_by) with a CHECK constraint holding the workbook's
FIFTEEN sheet names as the only legal states, and `hotel_contacts` (hotel_id,
slot, person_name, phone, phone2, email) - a child table because most sheets
give three contact columns and Selangor four. Deletes are soft (is_active = 0).
0112 seeds 442 hotels and 690 contacts with explicit ids and INSERT OR IGNORE,
so it is safe to re-apply; phone numbers were rewritten into Malaysian form on
the way in and the text was folded to ASCII for the remote D1 parser.

| Version | Change |
|---|---|
| v1.100.0 | 0111 — hotels + hotel_contacts, state as a closed list. |
| v1.100.0 | 0112 — 442 hotels and 690 contacts, seeded. |
