...
/* ---- the seven the audit found, named so they cannot regress quietly ---- */
{
  ...
  for (const [what, src, re] of [
    ...
    /* v1.105.0 — the same toast now says "Kept — no signal" when the request
       is waiting on the phone (the outbox), and "Leave requested" otherwise.
       Either way it speaks. */
    ["applying for leave", page, /showLeaveToast\(\s*\n?\s*(?:res\.queued \? L\("Kept — no signal"[^:]*: )?L\("Leave requested"/],
