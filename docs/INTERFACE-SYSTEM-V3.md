...
2. **A row, a stack, a grid of cards, an icon, a muted line, a rhythm step:** the layout and type primitives in `erp-v3.css` (section *LAYOUT AND TYPE PRIMITIVES*):
   ...
   - **inside a table cell**, text breaks on WORDS (`overflow-wrap: break-word`), never `anywhere`. `anywhere` drops a cell's minimum content contribution to a single character and the automatic table layout will squeeze the column that far — v1.174.4: the Attendance register rendered its header as S/T/A/F/F and a staff name one letter per line. An **identifier** in a scrolling table — a name, an account code, a document reference — is `tdOneLine` / `thOneLine` (one line, the table scrolls); a number is `tdR2` / `thR2`, which already does.
...
**The Dashboard's zones are ordered per job (v1.175.0).** The five zones are named elements — `day` (the shift hero), `desk` (One Desk + the watchers), `month`, `company` (TradingDesk pulse), `around` — and `DASHBOARD_ZONES` in `components/portal/dashboard.tsx` maps three jobs onto three orders: **business** (the executive tier: company, desk, day, month, around), **sales** (desk, company, day, around, month), **shift** (day, desk, month, company, around — unchanged since v1.115.0). The job is chosen by role *and* by what the person may open. **Ordering grants nothing:** each zone keeps its own gate, so the layout can be a list of elements rather than a second set of role checks. The shift hero is never hidden — it takes `heroCompact` when it is not the lead, and On Shift keeps a bottom-bar stop for every role. A new dashboard zone joins the five names and all three orders; guard #71 fails a map that drops one.

**A row of the desk is `erp-list-row` + `erp-row-lead` + `erp-row-actions`**, with `erp-dot` (a named status dot) at its head and the next action under the status line. Only an unclaimed new enquiry carries an inline button, because only that row shows the evidence its decision needs.

**The command deck (v1.177.0) — the dark ground is the portal's look.** The surface ladder is `--background` (#0a1120) → `--card` (#16213a) → `--erp-surface-raised` (#1d2b47), and it must run that way round: `--secondary` is the correct ground on LIGHT and is *lighter* than a card on dark, which is what made every card read as a hole punched in the page and every shadow pointless. On a dark ground the thing that sells elevation is `--erp-hairline` — `inset 0 1px 0` of light along the top edge — not the shadow behind it. The brand reaches every tab through the small-caps zone caption (gold, with a gradient rule) rather than by recolouring cards. Feedback is `transform`/`opacity` only — 96% on press, a lift on a stat, and a gold glow BEHIND the 2px focus outline, never instead of it. A tab arrives with an 8px, 200ms lift. All of it is off under `prefers-reduced-motion` with colours and the focus ring unchanged. The theme is applied by a pre-paint script in `app/layout.tsx`, gated on `location.pathname` so the public site stays light, and dark is the portal's default; `app/portal/page.tsx` must make the same `!== "light"` test or the theme flips after hydration.

**The five defects a probe finds and the reader does not (v1.176.1).** `tests/browser/ui-audit.py` walks every tab in WebKit at 390px and reports **ECHO** (a zone caption and the card title under it are the same words), **REPEAT** (one card prints one phrase twice), **SHRED** (an element rendering under three characters per line), **TRAP** (a scroll container nested inside the page) and **ORPHAN** (a tile grid ending in one narrow tile). None of them overflows, so `webkit-overflow.py` correctly reports 0 for every one. Run all three probes after any layout, wrapping, table or card change. Four rules came out of the first run and are now the system's: **`.erp-chip` is one line** (if it cannot fit, its column scrolls); **a figure and its unit are one word** — `.erp-nowrap`, and automatically for any `span` among `.erp-row-actions`, which is the other half of v1.174.3 (the group wraps, the item does not); **an odd set's last tile takes the whole row on a phone**; **`.tbl-sticky-lead` cells are top-aligned**. And one prohibition: **never nest a vertical scroller inside the page on a phone** — `max-h-* overflow-y-auto` around a table or list steals the scroll gesture and the page will not move. A wide table is `.erp-table-scroll`, sideways only.

**One home per workflow (v1.176.0).** A workflow is completed in exactly ONE place; every other surface may state a figure and link to it. **The Desk is a page** (`components/portal/desk-page.tsx`), not a Dashboard anchor — *a page header must name the destination the person chose*, and a bottom-bar stop that scrolls another page breaks that. So: the Desk owns the queue and the Dashboard shows `DeskSummary` (three figures, two preview rows, one link); **Tasks owns the task list** and the Desk's task zone is a count and a button; the bell owns notifications with read/unread state and links and is never a second approval queue; management rule configuration is a *Setup* zone at the foot of the Desk, out of the everyday queue. Two more rules fall out of it: **a status line says what the item IS (`sub`), the action line says what YOU do (`next`)** — never the same sentence twice — and **repeated findings of one rule group into a single expandable line with a count** (`WatchersCard section="findings"`). Counts on two surfaces come from one `useCachedApi` path (`useDeskData()`), so they cannot disagree.

**Where each tab now stands (all 32 in `ALL_TABS`; Dashboard and On Shift are the reference).**

| Tab | Zones (top → bottom) | Figures | Pill row |
|---|---|---|---|
| Desk | At a glance · Needs your decision · Needs attention · Your tasks · Setup (v1.176.0) | decisions (+ oldest waiting) / attention — each zone caption carries its own count, so no card repeats it (v1.176.1) | — |
