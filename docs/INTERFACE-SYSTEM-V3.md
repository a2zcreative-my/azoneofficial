...
**A row of the desk is `erp-list-row` + `erp-row-lead` + `erp-row-actions`**, with `erp-dot` (new, a named status dot) at its head and the next action under the status line. One Desk shows *Your decision* above *Your work*; only an unclaimed new enquiry carries an inline button, because only that row shows the evidence its decision needs.
