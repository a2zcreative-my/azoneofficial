...
**Last revised:** 31 August 2026 · rev 1.13 (see [Revision log](#revision-log))
...
| `0097` | `leave_recorded_direct` - an unpaid day recorded by management, not applied for |
| `0098` | `web_order_tracking_url` - the courier link, mirrored from the shop |
| `0099` | `shift_patterns` - working-hour schedules + effective-dated assignments |
| `0100` | `attendance_pending` - a forgotten punch waits for the CEO |
| `0101`+ | free. Tracks C/D/E take their numbers from here, in ship order. Track V needs **none**. |
...
| **OD-30** | **Which photo goes on a card page?** (Track V) | (a) Reuse the portal staff photo (`photo_key`, already on the badge card). (b) Commission studio shots first. | **(a).** A card that ships is worth more than a card waiting on a photographer, and the page reads a field — swap them individually later. **Decided 30-08-2026: (c) — the photo, copied in.** The CEO supplied the three headshots the same day, so v1.71.0 ships them as STATIC files in `public/cards/`, cropped square on the face and framed identically across the three. Not fetched from the portal: `/api/v1/media/file/<key>` would put a grey box on a client's screen whenever the API had a bad day, and would make every staff photo publicly fetchable by key. The explicit two-letter monogram (`AF`, `IZ`, `ZO`) remains the fallback for a record with no photo. |
...
| Date | Rev | Change | By |
|---|---|---|---|
| 2026-08-31 | 1.13 | **Buttons that answer** (v1.77.0). CEO: *"on the Task, there is no popup box to show if there is any task successfully deleted... you require to audit all the files in this project to ensure that all is globally!"* - then two things plainly broken within the hour. **The unpaid-day chip 500'd**: the route read `WORK_DAY_MINUTES`, a `const` declared about a hundred lines further down the SAME function - a temporal dead zone, ReferenceError. Second time in two days (`/payroll/absences` was the same shape). Hoisted to module scope. **Offboard answered "Staff route not found"**: `staff-directory.tsx` builds requests with the staff prefix, so `/users/12/offboard` went out as `/api/v1/staff/users/12/offboard` while the route lives at `/api/v1/users/12/offboard` beside the other account-lifecycle routes (it kills sessions and clears 2FA). The handler was perfect; the address was wrong. **The audit he asked for**: every mutating call in all 64 client files - nine actions moved money, time off or a record and then said nothing (task delete, leave decide, the CEO leave override, an OT decision, removing a commission rule, crediting a supplier return, recording a replacement, deleting a supplier return, deleting a company event, deleting a staff document). All now report both outcomes. **Three guards.** #25 `action-feedback`: a DELETE must report, and anything gated behind a confirm must report - its first draft read forty lines around the call and PASSED a deliberately silent delete because a helper underneath it happened to show a message, so it reads the enclosing function now. #26 `api-routes`: resolves all 342 client API calls to the path they put on the wire and checks each against the routes the worker serves - the only thing that could have caught Offboard, since both sides are strings and the handler genuinely exists; its own first draft passed because `path.startsWith("/api/v1/staff/")` in index.ts was read as a route rather than as the DOOR to another dispatcher. #14 `worker-compile-gate` **tightened**: it already ran real tsc over the API and already saw the crash as `TS2448`, but was counting it among the pre-existing strict-mode complaints it ignores by design - *used before its declaration* is not an opinion about strictness, it is the 19-08 outage wearing a different hat, and is fatal now (`TS2448`/`TS2454`, zero pre-existing). **A month of unpaid leave still paid five Saturdays** (CEO: *"Zul Hisyam should entitle 2 PH but seem like the payroll make it around 5++ which is not correct!"*). Both halves exact: absent all 19 of August's working days, he lost 19/26 and kept 7/26 - RM 538.46 - being his 2 public holidays plus 5 Saturdays. A DIVISOR MISMATCH, not a rounding error: 1/26 assumes a six-day week and this company works five, so no number of unpaid working days can reach the whole salary. Rule chosen (of three): **a week in which every one of that person's working days is unpaid also loses that week's rest days** - no cliff, unlike "absent all month = nothing", and partial months taper. **Public holidays are never touched** by the rule or by the cap beneath it (s.60D(2) removes holiday pay only for absence WITHOUT consent; recorded unpaid leave has consent). Zul RM 538.46 -> RM 153.85; 15-of-19 RM 846.15 -> RM 384.62; a scattered 2.75 days UNCHANGED. The cap is a second fix: incomplete-month + unpaid could exceed the basic and print a negative payslip. **Working a public holiday now pays** (CEO: *"if they are working on Public Holiday, then only will be paid as double... we need to follow on the regulation"*) - it paid NOTHING extra before. Rate confirmed with him against the Act, not the word "double": s.60D(3)(a)(i), TWO days' ORP in addition to the holiday pay (+RM 153.85 on RM 2,000); part-timers get a second RM15/h on those hours (Part-Time Employees Regs 2010). Only an APPROVED clock-in on a `public`/`replacement` calendar date counts - a `company` day off carries no statutory premium. `phWorkResolver` feeds the payslip, the panel, recompute and the hourly save; `PART_TIME_LH_RATE_CENTS` hoisted to module scope because the resolver runs above where it was declared (TDZ). **`employedDays` ignored `rejoined_on`** - a returning employee was prorated away from their old leaving date, silently; threaded through all six call sites. **Five copies of the formula became one**: the payslip, `/payroll/recompute` (which WRITES net_cents) and the browser's three all now come through one resolver, with a runnable twin in `lib/payroll-days.ts` that guard #23 EXECUTES against the real August figures; the guard also fails on any per-day arithmetic reappearing in the panel. **And the reason is on the screen** - `- RM 1,052.63 auto` with the explanation in a hover tooltip is how an unexplained charge sat on Nurfarah's row; every automatic deduction now says what it is. Her row also showed **9 working days employed but 12 clocked** - impossible, one of her dates is wrong - now flagged on the row (warn, not block, per the CEO). Guard #20's four stale checks rewritten: they named the exact expressions and went red when the duplication they policed was REMOVED, which is a guard failing on an improvement. **Offboarding now asks for the last day** (CEO: *"offboard should I can insert the date of their resignation ... instead of capture to today date"*) - it stamped today, which is only right if somebody walks out the moment you press it, and `left_on` is what payroll prorates a final month on. Pre-filled with today in MALAYSIA time (the same expression the worker uses), cannot be submitted empty, read back in the confirmation; the server now 400s on a date it cannot parse instead of silently substituting today. `usePrompt` gained a date-only mode rather than a second bespoke modal. **Also: the Payroll tab was taking the better part of a minute and showing "TOTAL - 0 staff."** Two faults, both from the v1.76.0 shift work. Resolving one person's hours on one date is two D1 queries, and the absence scan did it inside two nested loops (person x day) while the attendance export did it per PUNCH - roughly five hundred sequential round trips before the page could draw a row. `shiftResolver()` now reads the whole schedule in TWO queries and answers in memory; `shiftOn` stays only for the single-punch classifier. Separately the panel `await`ed that scan mid-load while the table is drawn at the end, so the tab waited on a suggestion card to show a salary run. Guard #24 gained the invariant as something checkable rather than advice: **no shift lookup inside a loop** (brace-tracked), and the table must not await the scan. **Skeleton loading, both repos** (CEO: *"skeleton loading react doesnt accurately follow the width of my interface ... I want no loading without skeleton loading react ... audit all the files"*). `PortalSkeleton` still capped its canvas at 1440px after v1.74.0 removed that cap from AppShell and drew neither of the Dashboard's two side columns - a different shape from the app that replaced it, so the page jumped; it now carries AppShell's classes verbatim. Audit of every fetch-on-mount component: portal 89 (6 had skeletons; 5 said "Loading..." in words; 4 returned null), shop 12 (3 had skeletons; 10 "Loading..."; 1 spinner). All now show a shape-matched skeleton until the first request settles, empty states gated on it, one printed exemption (the login form). Done as a five-way parallel fan-out over disjoint files against a mechanical guard written FIRST, so "done" meant the guard passed rather than an opinion. Guard #28 `skeleton-loading` (portal) + the same in the shop's DEPLOY.bat (store v1.44.0). No migration. **28 guards.** | Claude |
| 2026-08-30 | 1.12 | **Working hours become a schedule** (v1.76.0) - Track B-8 arriving early, on CEO request. Hours were ONE constant (10:00-18:00 Mon-Fri) and the company had already moved its Friday finish by announcement without the code knowing, so every Friday was flagged against 18:00. `0099_shift_patterns` adds named patterns (start/finish per weekday, blank = rest day) plus effective-dated assignments - shared so a company-wide change is one edit, effective-dated so fixing hours today cannot re-flag a paid month. `shiftOn()` is the single resolver; the guard scans line by line to keep anything from reading the old constant again. **Weekend is now per person**: a blank day is that person's rest day, surfaced as `day_kind` in the register, the CSV and the payroll export. `0100_attendance_pending`: a clock-out with no clock-in was REFUSED, so a worked day could not be recorded and vanished from payroll - it is now stored pending and counts for NOTHING until the CEO approves it and sets the real time (rejecting deletes it; a zombie row gets counted by something later). Five counting queries exclude pending through one shared clause. Guard #24 `shift-schedule` (37 checks). | Claude |
| 2026-08-30 | 1.11 | **Payroll: the payable-days model** (v1.75.0). Auditing the CEO's payslip request found a LIVE over-deduction: the incomplete-month proration read DAYS CLOCKED, so a day of approved MEDICAL leave - paid by law - looked like absence and was charged at basic / working days. Nur Nasuha, August 2026, RM 105.26 on a slip that added up. **The two deductions now read sources that cannot overlap**: incomplete month from `joined_on`/`left_on` only, unpaid leave from explicitly recorded days only. Attendance never moves money by itself, so an existing staff member is mathematically un-proratable - no flag, the formula is the rule. Public holidays counted inside the employment span. `GET /payroll/absences` PROPOSES days with no clock-in (and days clocked short of 8h, break included) as one-click chips - never deducts, because a missing punch is a flat battery as often as an absence. `POST /attendance/unpaid` takes `hours_worked` -> quarter-day fractions (2h of 8 = 0.75 day); `leave_requests.days` was REAL since 0003 so no migration. Guard #23 `payroll-days` compiles `lib/payroll-days.ts` and RUNS it, reproducing the old 315.79 to prove which arithmetic ran, then asserting the new net. Guard #20's obsolete double-deduction checks were rewritten rather than deleted: the invariant is now that proration never reads the clock. | Claude |
| 2026-08-30 | 1.10 | **Attendance CSV + the portal fills the window** (v1.74.0). A **CSV button that exports the FILTERED rows**, not the month: the table and the button share one `exportRows()`, because two definitions of "visible" drift the first time a filter is added and the file then disagrees with the screen. New `lib/csv.ts` is the one builder - BOM (or Excel reads UTF-8 as the local code page), CRLF, real quoting, and **formula defusing** for a cell starting `=` `+` `-` `@`, which Excel executes on open. The inventory stock-count export, which had its own escaper, now goes through it too. Guard #22 `csv-export` compiles the module and checks the BYTES it produces rather than the shape of its source. Separately: `AppShell`'s 1440px canvas cap and `PORTAL_WIDTH`'s 1600px content cap are both removed - on a 1920 monitor they left a band of backdrop down each side. The rounded canvas, the p-5 backdrop, the rail, both side columns and every phone rule are untouched, and PORTAL_WIDTH stays ONE width in ONE place so it can be capped again with a single edit. | Claude |
| 2026-08-30 | 1.9 | **Web-order tracking finished** (portal v1.73.0 + store v1.43.0). Audit first: the courier and tracking-number controls have existed since v1.51.0 and J&T and Ninja Van were always in the list - the CEO could not see them because every order on his tab was CANCELLED (the boxes appear on a PAID order) and because the live API was still v1.32.1 until this morning, so the v1.51.0 route did not exist. Genuinely new: **feed C carries `tracking_url`**, built by the SHOP (`0098_web_order_tracking_url` mirrors it), making the Tracking column a real link; **`update_tracking`** (store, shipped-only, bumps `updated_at`, writes the correction into the order history) so a typo off a parcel label is no longer permanent; and a **WhatsApp the tracking** button, because marking a parcel shipped tells the shop page and nobody else while OD-12 (outbound email) is still open. Guard #21 `web-order-tracking` bans courier domains repository-wide - the only way to keep ONE courier map honest across two repositories that cannot import from each other - and store guard `order-tracking` (23 checks) holds the other end, wired into the store DEPLOY.bat. Bridge spec gains section C2 documenting every order action. | Claude |
| 2026-08-30 | 1.8 | **CEO powers on Tasks, Leave and Attendance** (v1.72.0). `task_delete` and `unpaid_leave` join `claims_decide` and `leave_entitlement` in the CEO-only tier. **Task delete** removes `task_items`, `task_events`, `task_comments` and `task_blocks` by hand (no ON DELETE CASCADE anywhere, and the roster reads `task_blocks` by DATE, so an orphan block would occupy a working week forever), notifies both parties and audits the TITLE. **Leave override** (`override: true`) takes a request from any stage straight to approved, refused to everyone but the CEO, never on your own application and never on a closed one - with no new column, because `hr_by`/`preapp_by` staying NULL beside a `final_by` IS the record that stages were skipped, and the printed form already renders it. **Unpaid leave from Attendance** stores the day as an approved unpaid-leave request (`0097_leave_recorded_direct`) instead of a new table, so the payslip, the payroll table and the Leave tab keep one source of truth - and payroll needed NO change, having deducted at the EA 1955 s.60I 1/26 rate since v1.4.79. Undo is restricted to `recorded_direct = 1` so it can never erase a staff member's own application. Guard #20 `unpaid-leave` (46 checks): permissions asserted as SETS, the cascade table by table, and the 1/26 rate at EVERY site that computes it - the first draft checked only one of the five and passed with another changed to 22, which is an underpaid salary no screen would show. | Claude |
