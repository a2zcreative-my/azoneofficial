...
type PendingPunch = { id: number; user_id: number; name: string; type: string; created_at: string; offline_sent_at?: string | null };
            ...
            {pending.map((pp) => (
              <div key={pp.id} className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">
                  ...
                  {" "}{pp.offline_sent_at ? L("pressed", "ditekan") : L("claimed", "didakwa")} {utcToMytLocal(pp.created_at).replace("T", " ")}
                  {/* v1.105.0 - a punch the phone kept and sent later: the
...
export function ClaimsPanel({ userId = 0, role = "" }: { userId?: number; role?: string }) {
  ...
  const submit = async () => {
    ...
    /* v1.105.0 - no signal: the claim is kept on the phone (lib/outbox.ts)
       ...
       which does not exist yet, so it cannot ride along - say so, and point
       at where to attach it later. */
    if (res.queued) {
      showToast(L("Kept — no signal", "Disimpan — tiada isyarat"),
        receipt
